...
class ActiveGenomeIndexContractFixtureMixin:
    ...
    def _assert_clinvar_payloads_are_real_alleles(self, matches_path: Path, *, expected_format: str) -> None:
        ...
        for payload in payloads:
            ...
            provenance_agi_record = payload["match_provenance"]["agi_record"]
            ...
            self.assertIn("agi_record_info", sample)
            ...
            if expected_format in {"vcf", "gvcf", "bam", "fastq", "genome"}:
                ...
                self.assertEqual(sample["agi_record_ref"], sample["ref"])
                self.assertEqual(sample["agi_record_alt"], sample["alt"])
                self.assertEqual(sample["record_kind"], "variant_call")
                self.assertEqual(provenance_agi_record["record_kind"], "variant_call")
            else:
                ...
                self.assertEqual(provenance_agi_record["source_format"], expected_format)
                ...
                self.assertEqual(sample["agi_record_ref"], ".")
                self.assertEqual(sample["agi_record_alt"], ".")
                self.assertEqual(sample["agi_record_format"], "GT_ARRAY")
                self.assertEqual(
                ...
                self.assertEqual(provenance_agi_record["ref"], ".")
                self.assertEqual(provenance_agi_record["alt"], ".")
                self.assertEqual(provenance_agi_record["genotype"], genotype)
                self.assertEqual(provenance_agi_record["record_kind"], "array_call")
                self.assertEqual(provenance_agi_record["observed_alleles"], list(genotype))
