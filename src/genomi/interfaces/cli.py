...
def _add_static(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    init = commands.add_parser("init", help="Create an AGI run layout from a raw VCF intake source.")
    ...
    run = commands.add_parser("run", help="Run raw VCF intake into AGI-backed static evidence materialization.")
    ...
    match = commands.add_parser("match-clinvar", help="Match variants from the Active Genome Index derived from the intake VCF path.")
    ...
    query = commands.add_parser("query", help="Query Active Genome Index and static evidence.")
