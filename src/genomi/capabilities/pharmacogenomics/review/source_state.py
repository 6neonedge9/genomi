...
def _evidence_envelope(
    *,
    query: JsonObject,
    source_availability: JsonObject,
    evidence_components: JsonObject,
    evidence_state: JsonObject,
    evidence_matrix_traceability: JsonObject,
    sample_context_requested: bool,
    clinical_context_requested: bool,
) -> JsonObject:
    components = list(evidence_components.get("items") or [])
    component_states = {
        str(item.get("id")): str(item.get("state") or "unknown")
    ...
    checked_sources = [
        {
            ...
            "raw_call_count": item.get("raw_call_count"),
        }
        for item in source_availability.get("sources") or []
    ]
    source_coverage_status = str(source_availability.get("status") or "unknown")
    source_warning_count = int(source_availability.get("warning_source_count") or 0)
    source_unavailable_count = int(source_availability.get("unavailable_source_count") or 0)
    has_source_coverage_warning = bool(
        source_warning_count
        or source_unavailable_count
        or source_coverage_status in {"partial_source_unavailable", "source_evidence_available_with_warnings", "queried_no_source_evidence_with_warnings"}
    )
    if evidence_state.get("has_public_pgx_evidence") and not unresolved:
        status = "bounded_evidence_with_source_warnings" if has_source_coverage_warning else "bounded_evidence_ready"
    elif evidence_state.get("has_public_pgx_evidence"):
        status = "bounded_evidence_with_open_items"
    elif source_availability.get("status") == "source_unavailable_no_evidence":
    ...
    return {
        ...
        "checked": {
            "public_sources": checked_sources,
            "source_availability_status": source_coverage_status,
            ...
            "evidence_matrix_source_ids": evidence_matrix_traceability.get("source_ids"),
        },
        "traceability": {
            "all_items_have_stable_ids": evidence_matrix_traceability.get("all_items_have_stable_ids"),
            "all_items_have_verification": evidence_matrix_traceability.get("all_items_have_verification"),
            "verification_status_counts": evidence_matrix_traceability.get("verification_status_counts"),
            "source_traceable_item_count": evidence_matrix_traceability.get("source_traceable_item_count"),
            "missing_source_url_item_count": evidence_matrix_traceability.get("missing_source_url_item_count"),
            "local_sample_item_count": evidence_matrix_traceability.get("local_sample_item_count"),
            "user_provided_unverified_item_count": evidence_matrix_traceability.get("user_provided_unverified_item_count"),
            "citation_count": evidence_matrix_traceability.get("citation_count"),
            "pmids": evidence_matrix_traceability.get("pmids"),
        },
